...
{
  pub fn list(labels: &'a [L], context: &'a TcliContext) -> Self {
  ...
  pub fn show(labels: &'a [L], context: &'a TcliContext) -> Self {
    let mut tabled_builder = TabledBuilder::default();
    if context.show_headers() {
...
impl<'a> StringTableBuilder<'a> {
  pub fn new(labels: &'a [&'a str], context: &'a TcliContext) -> Self {
    let mut tabled_builder = TabledBuilder::default();
    if context.show_headers() {
...
pub fn print_ids(target_id: String, ids: Vec<String>, context: &TcliContext) {
  let mut tabled_builder = TabledBuilder::default();
