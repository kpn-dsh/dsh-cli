...
use crate::global_options::{TENANTS_ALL_FLAG, TENANTS_OPTION};
...
/// Create client for explicit platform and tenant for single-sign-on
///
/// ## Parameters
/// * `api_platform`
/// * `api_tenant`
...
pub(crate) async fn create_client_access_token_from_platform_tenant(api_platform: &DshPlatform, api_tenant: &str, context: &Context) -> DshCliResult<Option<DshApiClient>> {
  if *context.authentication_method() == AuthenticationMethod::SingleSignOn {
    if supports_dsh_directory() {
      match get_access_token(api_platform.clone()).await {
        ...
        Ok(None) => {
          context.print_warning(format!(
            "please log in to platform '{}' using the 'dsh login {}' command",
            api_platform,
            api_platform.alias()
          ));
          Ok(None)
      ...
      err!("single-sign-on requires dsh directory to be enabled")
    }
  } else {
    err!("robot authentication method not allowed")
  }
}
...
/// Create client for single-sign-on
///
/// ## Parameters
/// * `matches`
/// * `requirements`
...
async fn create_clients_single_sign_on(matches: &ArgMatches, requirements: &Requirements, context: &Context) -> DshCliResult<Option<Vec<DshApiClient>>> {
  if !requirements.authentication_method_allowed(&AuthenticationMethod::SingleSignOn) {
    return err!("single-sign-on authentication method not allowed");
  }
  if !supports_dsh_directory() {
  ...
  if matches.get_flag(TENANTS_ALL_FLAG) {
    ...
    match matches.get_one::<String>(TENANTS_OPTION) {
...
/// Create multiple clients for single-sign-on
///
/// ## Parameters
/// * `target_platform`
/// * `target_tenant_names`
