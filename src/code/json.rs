...
pub(crate) fn generate_json_configuration(bundle_configuration: &ProxyCertificateBundleConfig, bundle_directory: &str, context: &Context) -> DshCliResult<Option<String>> {
  ...
  let json_filename = json_configuration_filename(bundle_configuration);
  fs::write(&json_filename, &json)?;
  context.print_outcome(format!("created json configuration file '{}'", json_filename));
  Ok(None)
}

pub(crate) fn json_configuration_exists(bundle_configuration: &ProxyCertificateBundleConfig) -> DshCliResult<bool> {
  Ok(exists(json_configuration_filename(bundle_configuration))?)
}
...
fn json_configuration_filename(bundle_configuration: &ProxyCertificateBundleConfig) -> String {
  format!("{}-configuration.json", bundle_configuration.proxy_name)
}
