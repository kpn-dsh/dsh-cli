...
use crate::processor::{PipelineProcessorName, TaskId};
...
impl ServiceClient<'_> {
  ...
  pub async fn _get_tasks(&self, service_name: &PipelineProcessorName) -> Result<HashMap<TaskId, TaskStatus>, String> {
    // TODO Panics
  ...
  ///
  /// ```
  pub async fn get_deployed_service_configuration(&self, service_name: &PipelineProcessorName) -> Result<Application, String> {
    let target_client = self.target_client_factory.client().await?;
    self.process(
      target_client
        .client()
  ...
  ///
  /// ```
  pub async fn get_service_configuration(&self, service_name: &PipelineProcessorName) -> Result<Application, String> {
    let target_client = self.target_client_factory.client().await?;
    self.process(
      target_client
        .client()
  ...
  /// }
  /// ```
  pub async fn get_service_status(&self, service_name: &PipelineProcessorName) -> Result<AllocationStatus, String> {
    let target_client = self.target_client_factory.client().await?;
    self.process(
      target_client
        .client()
  ...
  /// ]
  /// ```
  pub async fn get_all_derived_task_ids(&self, service_name: &PipelineProcessorName) -> Result<ChildList, String> {
    let target_client = self.target_client_factory.client().await?;
    self.process(
      target_client
        .client()
  ...
  /// }
  /// ```
  pub async fn get_derived_task_status(&self, service_name: &PipelineProcessorName, task_id: &TaskId) -> Result<TaskStatus, String> {
    let target_client = self.target_client_factory.client().await?;
    self.process(
      target_client
        .client()
  ...
  /// ```
  ///
  pub async fn get_task_status_description(&self, service_name: &PipelineProcessorName, task_id: &TaskId) -> Result<AllocationStatus, String> {
    let target_client = self.target_client_factory.client().await?;
    self.process(
      target_client
        .client()
  ...
  /// }
  /// ```
  pub async fn get_task_state(&self, service_name: &PipelineProcessorName, task_id: &TaskId) -> Result<Task, String> {
    let target_client = self.target_client_factory.client().await?;
    self.process(
      target_client
        .client()
