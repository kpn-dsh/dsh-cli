...
impl<'a> DshAppRealizationRegistry {
  ...
  pub(crate) fn dshapp_descriptors(&self, engine_target: &EngineTarget) -> Vec<ProcessorDescriptor> {
    self.dshapp_realizations.values().map(|realization| realization.descriptor(engine_target)).collect()
  }
}
